...
func (NATPreOriginalDst) ContainerAction(ip net.IP) error {
	...
	addrs, err := getInterfaceAddrs(false)
	if err != nil {
		return err
	}
	return listenForRedirectedConn(addrs)
}
...
func (NATOutOriginalDst) ContainerAction(ip net.IP) error {
	...
	if err := listenForRedirectedConn([]net.IP{ip.To4()}); err != nil {
		return err
	}
	return <-connCh
}
...
func listenForRedirectedConn(originalDsts []net.IP) error {
